...
pub fn deserialize(bytes: &[u8], schema: &TableSchema) -> Result<Vec<Row>, Box<dyn Error>> {
    if bytes.len() < 5 {
        return Err("Avro too short".into());
    }
    let payload = &bytes[5..];
    ...
    for (i, col) in schema.columns.iter().enumerate() {
        if pos >= payload.len() {
            row.set(i, seatunnel_api::Field::Null);
            continue;
        }
        let (field, consumed) = match col.column_type {
            seatunnel_api::ColumnType::Int64 => {
                if pos + 8 > payload.len() {
                    ...
                    (
                        seatunnel_api::Field::Int64(i64::from_le_bytes([
                            payload[pos],
                            payload[pos + 1],
                            payload[pos + 2],
                            payload[pos + 3],
...
pub fn serialize(schema: &TableSchema, row: &Row) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    for (i, col) in schema.columns.iter().enumerate() {
        ...
        match col.column_type {
            seatunnel_api::ColumnType::Int64 => {
                if let seatunnel_api::Field::Int64(v) = field {
