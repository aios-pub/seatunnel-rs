...
pub fn deserialize(bytes: &[u8], schema: &TableSchema) -> Result<Vec<Row>, Box<dyn Error>> {
    ...
    let before_map: serde_json::Map<String, Value> = obj.get("before")
        .and_then(|v| v.as_object())
        .cloned()
        .unwrap_or_default();
    let after_map: serde_json::Map<String, Value> = obj.get("after")
        .and_then(|v| v.as_object())
        .cloned()
        .unwrap_or_default();
    ...
    match op {
        ...
        "u" => {
            ...
            let mut before_row = Row::new(seatunnel_api::RowKind::UpdateBefore, schema.column_count());
            for (i, col) in schema.columns.iter().enumerate() {
                before_row.set(i, map_to_field(&before_map, &after_map, &col.name, false)?);
            }
            rows.push(before_row);

            let mut after_row = Row::new(seatunnel_api::RowKind::UpdateAfter, schema.column_count());
            for (i, col) in schema.columns.iter().enumerate() {
...
fn map_to_field(before: &serde_json::Map<String, Value>, after: &serde_json::Map<String, Value>, name: &str, prefer_after: bool) -> Result<seatunnel_api::Field, Box<dyn Error>> {
...
.unwrap();
        ...
        assert_eq!(*rows[0].get(1), seatunnel_api::Field::String("old".to_string()));
        assert_eq!(*rows[1].get(1), seatunnel_api::Field::String("new".to_string()));
    }
    #[test]
    fn test_debezium_delete() {
        let rows = deserialize(b"{\"before\":{\"id\":2},\"after\":null,\"op\":\"d\"}", &make_schema())
.unwrap();
        let row = &rows[0];
        assert_eq!(rows.len(), 1);
        let row = &rows[0];
