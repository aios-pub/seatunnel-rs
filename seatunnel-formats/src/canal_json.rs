...
pub fn serialize(schema: &TableSchema, row: &Row) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    for (i, col) in schema.columns.iter().enumerate() {
        if i < row.field_count() {
            data_obj.insert(col.name.clone(), field_to_json(row.get(i))?);
        }
    }
    obj.insert(
        "data".to_string(),
        Value::Array(vec![Value::Object(data_obj)]),
    );
...
mod tests {
    ...
    fn test_canal_insert() {
        let rows = deserialize(
            b"{\"_type\":\"INSERT\",\"_ts\":1000,\"data\":[{\"id\":1,\"name\":\"alice\"}]}",
            &make_schema(),
        )
        .unwrap();
        let _row = &rows[0];
