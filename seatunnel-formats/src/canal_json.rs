...
pub fn deserialize(bytes: &[u8], schema: &TableSchema) -> Result<Row, Box<dyn Error>> {
    ...
    let kind = match obj.get("_type").and_then(|v| v.as_str()) {
        Some("INSERT") | Some("UPDATE") => seatunnel_api::RowKind::Insert,
    ...
    let data = match obj.get("data").and_then(|v| v.as_array()).and_then(|a| a.first()).and_then(|v| v.as_object()) {
        Some(m) => m,
        None => &empty_map,
    };
...
pub fn serialize(schema: &TableSchema, row: &Row) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    obj.insert("_type".to_string(), Value::String(match row.kind { seatunnel_api::RowKind::Delete => "DELETE", _ => "INSERT" }.to_string()));
...
mod tests {
    ...
    fn test_canal_insert() {
        let row = deserialize(b"{\"_type\":\"INSERT\",\"_ts\":1000,\"data\":[{\"id\":1,\"name\":\"alice\"}]}", &make_schema()).unwrap();
        assert_eq!(row.kind, seatunnel_api::RowKind::Insert);
        assert_eq!(*row.get(0), seatunnel_api::Field::Int64(1));
    }
    #[test]
    fn test_canal_delete() {
        let row = deserialize(b"{\"_type\":\"DELETE\",\"data\":[{\"id\":2}]}", &make_schema()).unwrap();
