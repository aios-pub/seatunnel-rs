    ...
    while pos < bytes.len() {
        ...
        let (field, consumed) = match wire_type {
            ...
            2 => {
                ...
                let read_pos = pos + c1;
                if read_pos + len > bytes.len() {
                    (seatunnel_api::Field::Null, 0)
                } else {
                    let s = String::from_utf8_lossy(&bytes[read_pos..read_pos + len]).to_string();
                    let f = match col.column_type {
                        seatunnel_api::ColumnType::Int64 => match s.parse::<i64>() {
                            Ok(v) => seatunnel_api::Field::Int64(v),
                            Err(_) => seatunnel_api::Field::String(s),
                        },
                        seatunnel_api::ColumnType::Bool => match s.to_lowercase().as_str() {
                            "true" | "1" => seatunnel_api::Field::Bool(true),
                            _ => seatunnel_api::Field::Bool(false),
                        },
...
mod tests {
    ...
    fn test_protobuf_roundtrip() {
        ...
        let rows = deserialize(&bytes, &schema).unwrap();
