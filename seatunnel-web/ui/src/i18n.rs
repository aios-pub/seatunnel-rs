...
const DICT: &[(&str, &str, &str)] = &[
    ...
    ("topbar.theme", "Toggle dark mode", "切换暗色模式"),
    ("wd.title", "Worker {}", "节点 {}"),
    ("wd.tasks", "Tasks owned by this worker", "该节点持有的任务"),
    ("wd.no_tasks", "No running tasks on this worker.", "该节点暂无运行中任务。"),
    ("wd.back", "← Back to cluster", "← 返回集群"),
    ("logs.title", "Node logs", "节点日志"),
    ("logs.select", "Log file", "日志文件"),
    ("logs.level", "Level", "级别"),
    ("logs.all", "All", "全部"),
    ("logs.search_ph", "Filter lines…", "过滤行…"),
    ("logs.tail", "Tail lines", "行数"),
    ("logs.no_dir", "No log directory configured on this node.", "本节点未配置日志目录。"),
    ("logs.no_files", "No log files yet.", "暂无日志文件。"),
    ("logs.pick", "Select a file to view.", "请选择要查看的日志文件。"),
    ("logs.download", "Download", "下载"),
    // Login
    ...
    ("jd.editor_title", "Edit job configuration", "编辑作业配置"),
    ("jd.editor_format", "Format", "格式"),
    ("jd.fmt_auto", "Auto-detect (JSON or YAML)", "自动检测（JSON 或 YAML）"),
