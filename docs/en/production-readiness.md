...
| G | **Full service restart** (whole cluster / hybrid node down, then rebooted with the same state dir) → non-terminal jobs re-dispatched automatically; tasks resume from their last checkpoint (batch reruns from scratch) | ✅ | `Restart recovery: ... non-terminal job(s) restored from durable state`, `Restart recovery: task ... released`, task RUNNING again on the re-registered worker |
...
- **Master HA (Stage 3)**: openraft consensus over the member list (voter counts 1/3/5; two rejected). The coordinator's durable mutations replicate through the Raft `Command` log; the leader term is the wire fencing term; election completes in ~2 s (one round; per-voter disjoint election windows prevent split-vote storms). Workers and clients (`-a addr1,addr2,...`) rotate addresses; follower masters serve no instructions and point workers at the leader via `leader_hint`.
...
- Change a running job's config with `seatunnel job update -c new.yaml -i <job-id>` or the web console's 编辑配置并重启 (cancel-with-exit-checkpoint → same-id resubmission → checkpoint restore). Do NOT run old and new configs in parallel — they double-consume the source. Cross-worker restore needs s3/master checkpoint storage.
- Restart a historical job as-is (same config, same id) with `POST /api/v1/jobs/{job_id}/restart` — the engine cancels a non-terminal incarnation first (exit checkpoint, ≤ the server-side cancel timeout), then resubmits from the retained config; tasks resume from their last checkpoint. Same safety rule as the update flow: on cancel timeout the restart aborts without resubmitting. Keep the same `<state-dir>` (and worker ids) across service restarts — that is where the durable Raft state and worker-local checkpoints live; a wiped state dir means cold starts.
