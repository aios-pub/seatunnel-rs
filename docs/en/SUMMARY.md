# Summary

- [Quickstart](quickstart.md)
- [Cluster HA Design (vs. Java Zeta)](cluster-ha-design.md)
- [Engine Configuration (seatunnel.yaml)](engine-config.md)
- [Production Readiness](production-readiness.md)

# Connectors

- [JDBC (MySQL / TiDB / PostgreSQL)](connectors/jdbc.md)
- [Kafka](connectors/kafka.md)
- [Redis](connectors/redis.md)
- [Elasticsearch](connectors/elasticsearch.md)
- [RabbitMQ](connectors/rabbitmq.md)
- [HTTP](connectors/http.md)
- [ClickHouse](connectors/clickhouse.md)
- [MySQL CDC](connectors/cdc-mysql.md)
- [TiDB CDC](connectors/cdc-tidb.md)
- [PostgreSQL CDC](connectors/cdc-postgres.md)

# Features

- [Multi-Pipeline Jobs (Fan-Out)](pipelines.md)
- [Startup Modes & Resuming from a Point in Time](startup-modes.md)
- [Schema Evolution](schema-evolution.md)
